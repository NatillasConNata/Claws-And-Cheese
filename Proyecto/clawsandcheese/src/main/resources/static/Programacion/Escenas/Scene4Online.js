    ...
    init() {
        ...
        this.functions = {
            ...
            update: function (message) {
                ...
                // CAJAS
                if (that.cajas) {
                    for (let i = 0; i < that.cajas.getChildren().length; i++) {
                        const caja = that.cajas.getChildren()[i];
        ...
        // Dependiendo del mensaje utiliza una función u otra
...
console.log("InicioCreate");
        ...
        //PLAYERS
        //ahora ambos tienen el mismo control
...
collectKey(player, key) 
{
    ...
    {
        //WEBSOCKETS
        if (this.controlled.player == "Cheese")
        {
            this.controlled.player = this.player1;
        }
        else if (this.controlled.player == "Claws")
        {
    ...
    //Averiguar si partida ya tiene dos jugadores
    /*getGame(id, callback) {
        $.ajax({
            method: 'GET',
